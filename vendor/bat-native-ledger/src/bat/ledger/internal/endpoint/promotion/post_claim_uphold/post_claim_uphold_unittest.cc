  ...
  ON_CALL(*mock_ledger_client_, LoadURL(_, _))
      .WillByDefault(Invoke(
          [](type::UrlRequestPtr request, client::LoadURLCallback callback) {
            ...
            response.body = R"(
{
    "message": "error linking wallet: unable to link wallets: unable to match wallets: wallets do not match",
    "code": 403
}
            )";
            callback(response);
...
TEST_F(PostClaimUpholdTest, ServerError403NonKYCdUser) {
  ON_CALL(*mock_ledger_client_, LoadURL(_, _))
      .WillByDefault(Invoke(
          [](type::UrlRequestPtr request, client::LoadURLCallback callback) {
            type::UrlResponse response;
            ...
            response.body = R"(
{
    "message": "error linking wallet: : user kyc did not pass",
    "code": 403
}
            )";
            callback(response);
  ...
  ON_CALL(*mock_ledger_client_, LoadURL(_, _))
      .WillByDefault(Invoke(
          [](type::UrlRequestPtr request, client::LoadURLCallback callback) {
            ...
            response.body = R"(
{
    "message": "error linking wallet: failed to verify transaction: failed to verify transaction",
    "code": 403
}
            )";
            callback(response);
