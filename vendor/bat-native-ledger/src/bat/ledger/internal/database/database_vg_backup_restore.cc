...
void DatabaseVGBackupRestore::BackUpVirtualGrants(
  ...
  command->command = R"(
    WITH aux AS (
      SELECT SUM(
               CASE
                 WHEN redeem_id IS NOT NULL AND redeem_id != '' THEN 1
                 ELSE 0
               END
             ) AS in_progress
  ...
  command->record_bindings = {
      ...
      type::DBCommand::RecordBindingType::STRING_TYPE,  // trigger_id
...
void DatabaseVGBackupRestore::OnBackUpVirtualGrants(
  ...
  if (!response ||
    ...
    BLOG(0, "DatabaseRewardsBackup:: response is wrong");
  ...
  if (records.size() == 1 && AllNULLRecord(records[0])) {
    BLOG(0,
         "DatabaseRewardsBackup: at least one contribution or SKU order is "
         "in progress");
  ...
  for (const auto& record : records) {
    ...
    vg->trigger_id = GetStringColumn(record_pointer, 1);
    vg->trigger_type =
        static_cast<type::CredsBatchType>(GetIntColumn(record_pointer, 2));
    vg->creds = GetStringColumn(record_pointer, 3);
    vg->blinded_creds = GetStringColumn(record_pointer, 4);
    vg->signed_creds = GetStringColumn(record_pointer, 5);
    vg->public_key = GetStringColumn(record_pointer, 6);
    vg->batch_proof = GetStringColumn(record_pointer, 7);
    vg->status =
        static_cast<type::CredsBatchStatus>(GetIntColumn(record_pointer, 8));
    vg->token_id = GetInt64Column(record_pointer, 9);
    vg->token_value = GetStringColumn(record_pointer, 10);
    vg->value = GetDoubleColumn(record_pointer, 11);
    vg->expires_at = GetInt64Column(record_pointer, 12);
    vg->redeemed_at = GetInt64Column(record_pointer, 13);
    vg->redeem_type =
        static_cast<type::RewardsType>(GetInt64Column(record_pointer, 14));
...
bool DatabaseVGBackupRestore::AllNULLRecord(
  ...
  return GetStringColumn(record_pointer, 0).empty() &&
         GetStringColumn(record_pointer, 1).empty() &&
         !GetIntColumn(record_pointer, 2) &&
         ...
         GetStringColumn(record_pointer, 7).empty() &&
         !GetIntColumn(record_pointer, 8) &&
         !GetInt64Column(record_pointer, 9) &&
         GetStringColumn(record_pointer, 10).empty() &&
         GetDoubleColumn(record_pointer, 11) == 0.0 &&
         !GetInt64Column(record_pointer, 12) &&
         !GetInt64Column(record_pointer, 13) &&
         !GetInt64Column(record_pointer, 14);
}
...
void DatabaseVGBackupRestore::RestoreVirtualGrants(
  ...
  for (auto creds_id_cit = vgs.cbegin(); creds_id_cit != vgs.cend();
    ...
    BindString(command.get(), 1, vg.trigger_id);
...
void DatabaseVGBackupRestore::OnRestoreVirtualGrants(
  ...
  if (!response ||
      response->status != type::DBCommandResponse::Status::RESPONSE_OK) {
    BLOG(0, "Couldn't restore virtual grants!");
    return std::move(callback).Run(type::Result::LEDGER_ERROR);
  }
