...
void BackupRestore::Restore(
  ...
  if (ParseVirtualGrantBodies(vg_bodies, vgs) &&
    ...
    ledger_->database()->RestoreVirtualGrants(std::move(vgs),
                                              std::move(callback));
  }
}
...
void BackupRestore::OnBackup(type::Result result,
  ...
  BLOG(0, "BackUpVirtualGrants() result: " << result);
  if (result == type::Result::LEDGER_OK) {
    BLOG(0, GetVirtualGrantBodies(vgs));
    BLOG(0, GetVirtualGrantSpendStatuses(vgs));
  }
}
...
std::string BackupRestore::GetVirtualGrantBodies(
  ...
  for (auto creds_id_cit = vgs.cbegin(); creds_id_cit != vgs.cend();
    ...
    vg_body.SetStringKey("trigger_id", vg->trigger_id);
...
bool BackupRestore::ParseVirtualGrantBodies(const std::string& json,
  ...
  for (const auto& vg_body : vg_bodies->GetList()) {
    ...
    auto* trigger_id = vg_body.FindStringKey("trigger_id");
    ...
    if (!creds_id || !trigger_id || !trigger_type || !creds || !blinded_creds ||
