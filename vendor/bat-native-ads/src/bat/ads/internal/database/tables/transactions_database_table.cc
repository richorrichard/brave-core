...
TransactionInfo GetFromRecord(mojom::DBRecord* record) {
  ...
  transaction.value = ColumnDouble(record, 2);
  transaction.confirmation_type = ConfirmationType(ColumnString(record, 3));
  transaction.redeemed_at = ColumnDouble(record, 4);
