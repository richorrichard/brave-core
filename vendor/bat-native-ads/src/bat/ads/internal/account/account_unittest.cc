...
class BatAdsAccountTest : public AccountObserver, public UnitTestBase {
 protected:
  ...
  void OnDepositedFunds(const std::string& creative_instance_id,
                        const AdType& ad_type,
                        const TransactionInfo& transaction) override {
...
TEST_F(BatAdsAccountTest, SetWallet) {
  ...
  // Act
  std::cout << "FOOBAR.D" << std::endl;
  account_->SetWallet(kWalletId, kWalletSeed);
  std::cout << "FOOBAR.E" << std::endl;
  ...
  account_->GetStatement(
      [](const bool success, const StatementInfo& statement) {
        ...
        expected_statement.next_payment_date =
            TimeFromString("5 January 2021", /* is_local */ true).ToDoubleT();
