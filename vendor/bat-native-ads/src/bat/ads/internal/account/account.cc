...
void Account::Credit(const std::string& creative_instance_id,
  ...
  transactions::Add(
      ...
      [=](const bool success, const TransactionInfo& transaction) {
        ...
        NotifyDepositedFunds(creative_instance_id, ad_type, transaction);

        NotifyStatementOfAccountsDidChange();

        confirmations_->Confirm(transaction.id, creative_instance_id, ad_type,
                                confirmation_type);
      });
}
...
void Account::NotifyDepositedFunds(const std::string& creative_instance_id,
                                   const AdType& ad_type,
                                   const TransactionInfo& transaction) const {
  for (AccountObserver& observer : observers_) {
    observer.OnDepositedFunds(creative_instance_id, ad_type, transaction);
  }
}
