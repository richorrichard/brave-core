import { convertMojoTimeToJS, formatDateAsRelative } from './datetime-utils'

describe('Convertting from MojoTime works', () => {
  test('0 epoch time value', () => {
    // Epoch time
    expect(convertMojoTimeToJS({ microseconds: 0 })).toEqual(new Date(null))
  })
  test('converts normal values ok', () => {
    ...
    expect(convertMojoTimeToJS({ microseconds })).toEqual(expectedDate)
  })
})
