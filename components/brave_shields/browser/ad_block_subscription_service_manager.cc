...
void AdBlockSubscriptionServiceManager::Init(ResourceProvider* resource_provider) {
...
void AdBlockSubscriptionServiceManager::CreateSubscription(
  ...
  auto subscription_service = std::make_unique<AdBlockEngineService>(task_runner_);
  UpdateSubscriptionPrefs(sub_url, info);

  auto subscription_source_provider = std::make_unique<AdBlockSubscriptionSourceProvider>(
          local_state_, GetSubscriptionPath(sub_url).Append(kCustomSubscriptionListText));
  subscription_service->Init(subscription_source_provider.get(), resource_provider_);

  // TODO I believe this will attempt to load a nonexistent file. The service
  // will request initial data, but the download has not yet started.
  // See the original `load_on_start_`.
