...
bool AdBlockCustomFiltersSourceProvider::UpdateCustomFilters(
  ...
  auto buffer = std::vector<unsigned char>(custom_filters.begin(), custom_filters.end());
  ProvideNewListSource(buffer);

  return true;
}

void RespondWithCustomFilters(base::OnceCallback<void(bool deserialize, const DATFileDataBuffer& dat_buf)> cb, std::string custom_filters) {
  auto buffer = std::vector<unsigned char>(custom_filters.begin(), custom_filters.end());
  std::move(cb).Run(false, buffer);
}

void AdBlockCustomFiltersSourceProvider::Load(base::OnceCallback<void(bool deserialize, const DATFileDataBuffer& dat_buf)> cb) {
  content::GetUIThreadTaskRunner({})->PostTaskAndReplyWithResult(FROM_HERE, 
          base::BindOnce(&AdBlockCustomFiltersSourceProvider::GetCustomFilters, base::Unretained(this)),
