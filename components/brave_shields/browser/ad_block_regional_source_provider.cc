...
AdBlockRegionalSourceProvider::AdBlockRegionalSourceProvider(component_updater::ComponentUpdateService* cus, const adblock::FilterList& catalog_entry): uuid_(catalog_entry.uuid), component_id_(catalog_entry.component_id), component_updater_service_(cus) {
  // TODO save catalog_entry.uuid as well
  RegisterAdBlockRegionalComponent(component_updater_service_, catalog_entry.base64_public_key, component_id_, catalog_entry.title, base::BindRepeating(&AdBlockRegionalSourceProvider::OnComponentReady, base::Unretained(this)));
}
