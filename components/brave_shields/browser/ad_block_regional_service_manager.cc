...
void AdBlockRegionalServiceManager::StartRegionalServices() {
  ...
  for (base::DictionaryValue::Iterator it(*regional_filters_dict);
    ...
    if (enabled) {
      ...
      if (catalog_entry != regional_catalog_.end() && existing_engine ==
              regional_services_.end()) {
        auto regional_source_provider = std::make_unique<AdBlockRegionalSourceProvider>(component_update_service_, *catalog_entry);
        auto regional_service = AdBlockEngineServiceFactory(
            task_runner_);
...
AdBlockRegionalServiceManagerFactory(PrefService* local_state, std::string locale, component_updater::ComponentUpdateService* cus, scoped_refptr<base::SequencedTaskRunner> task_runner) {
  return std::make_unique<AdBlockRegionalServiceManager>(local_state, locale, cus, task_runner);
}
