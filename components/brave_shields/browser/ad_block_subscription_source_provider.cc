...
AdBlockSubscriptionSourceProvider::AdBlockSubscriptionSourceProvider(
        PrefService* local_state, base::FilePath list_file) : /*local_state_(local_state),*/ list_file_(list_file) {}
...
void AdBlockSubscriptionSourceProvider::Load(base::OnceCallback<void(bool deserialize, const DATFileDataBuffer& dat_buf)> cb) {
  base::ThreadPool::PostTaskAndReplyWithResult(
      FROM_HERE, {base::MayBlock()},
      base::BindOnce(&brave_component_updater::ReadDATFileData, list_file_),
