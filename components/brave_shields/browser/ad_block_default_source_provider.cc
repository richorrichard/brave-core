...
AdBlockDefaultSourceProvider::AdBlockDefaultSourceProvider(component_updater::ComponentUpdateService* cus, base::RepeatingCallback<void(const std::string& regional_catalog)> regional_catalog_available_cb) : regional_catalog_available_cb_(std::move(regional_catalog_available_cb)) {
  RegisterAdBlockDefaultComponent(cus, base::BindRepeating(&AdBlockDefaultSourceProvider::OnComponentReady, base::Unretained(this)));
}

AdBlockDefaultSourceProvider::~AdBlockDefaultSourceProvider() {}

void AdBlockDefaultSourceProvider::OnComponentReady(const base::FilePath& path) {
...
void AdBlockDefaultSourceProvider::Load(base::OnceCallback<void(bool deserialize, const DATFileDataBuffer& dat_buf)> cb) {
...
void AdBlockDefaultSourceProvider::Load(base::OnceCallback<void(const std::string& resources_json)> cb) {
  if (component_path_.empty()) {
  ...
  base::ThreadPool::PostTaskAndReplyWithResult(
      FROM_HERE, {base::MayBlock()},
      base::BindOnce(&brave_component_updater::GetDATFileAsString, component_path_.AppendASCII(kAdBlockResourcesFilename)),
      std::move(cb));
}
